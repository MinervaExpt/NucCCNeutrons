...
namespace fit
{
  double LinearFit::functionToFit(const double binCenter, const double* pars) const override
  {
    const double slope = (pars[1] - pars[0]) / fSumBinWidths;
    return pars[0] + slope * binCenter;
  }

  double LinearFit::getSqErrOnFunction(const double binCenter, const double* /*pars*/, const double* parErrs) const override
  {
  ...
  void LinearFit::guessInitialParameters(ROOT::Math::Minimizer& min, const int nextPar, const std::vector<Sideband>& sidebands, const double POTRatio) const override
  {
